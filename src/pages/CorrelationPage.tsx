...
interface TodoTask { id: string; name: string; zone: string; }
interface TodoData { tasks: TodoTask[]; }
...
export default function CorrelationPage() {
  ...
  useEffect(() => {
    ...
    apiGet<TodoData>("todo", "tracker-todo-tasks", { tasks: [] } as any).then(d => setTodo(d.tasks || []));
  }, []);

  const dates = useMemo(() => {
    const arr: string[] = [];
    for (let i = WINDOW - 1; i >= 0; i--) arr.push(fmt(addDays(cursor, -i)));
    return arr;
  }, [cursor]);
  ...
  // Build correlation matrix: correlation items vs (correlation items + todo active tasks + health metrics), across ALL dates
  const correlations = useMemo(() => {
    // Build daily series per variable
    const series: Record<string, Record<string, number>> = {}; // varName -> date -> value

    // Correlation items → daily avg
    ...
    // Todo tasks: presence signal per day is not tracked; use zone (active if persistent/inprogress) - skip for now
    // Health metrics
    ...
    // Compute pairs (corr-item vs anything else)
    ...
    <div className="space-y-4 max-w-6xl mx-auto">
      ...
      <Card className="overflow-x-auto">
        <div className="flex items-center justify-between p-2 border-b bg-muted/30">
          <Button variant="ghost" size="icon" onClick={() => setCursor(addDays(cursor, -WINDOW))}><ChevronLeft className="h-4 w-4" /></Button>
          <div className="text-sm font-medium">{dates[0]} → {dates[dates.length - 1]}</div>
          <Button variant="ghost" size="icon" onClick={() => setCursor(addDays(cursor, WINDOW))}><ChevronRight className="h-4 w-4" /></Button>
        </div>
        <table className="w-full text-xs">
          <thead className="bg-muted/40">
            <tr>
              <th className="text-left p-2 font-medium min-w-[180px] sticky left-0 bg-muted/40">Item</th>
              {dates.map(d => (
                <th key={d} className="p-1 font-medium text-center border-l" colSpan={3}>
                  <div>{d.slice(5)}</div>
              ...
              <tr key={it.id} className="border-t">
                <td className="p-1 sticky left-0 bg-card">
                  <div className="flex items-center gap-1">
                    ...
                    <Input className="h-7 text-xs border-0 flex-1" value={it.name}
...
function ScoreCell({ date, slots, onSet }: { date: string; slots?: DailySlots; onSet: (slot: "m" | "n" | "e", v: number | null) => void }) {
  return (
    <>
      {SLOTS.map(s => {
        const v = slots?.[s];
        return (
          <td key={s} className="p-0.5 border-l text-center">
            <button
              className={cn(
                "w-6 h-6 text-[10px] rounded transition",
                v === 0 && "bg-muted",
                v === 1 && "bg-blue-500/30 text-blue-700 dark:text-blue-300",
                v === 2 && "bg-amber-500/40 text-amber-800 dark:text-amber-200",
                v === 3 && "bg-red-500/50 text-red-800 dark:text-red-100",
